...
describe('Fetch ESP deliverability metrics:', () => {
	...
	describe('Campaign endpoint', () => {
		const campaignFixtures = {
			ok: require('./fixtures/metrics-discoverability-campaign-200.json'),
			badRequest: require('./fixtures/metrics-discoverability-campaign-400.json'),
			serverError: require('./fixtures/metrics-discoverability-campaign-500.json')
		};

		const exampleMetrics = 'metrics=count_targeted,count_delivered,count_unique_confirmed_opened';
		const campaignUrl = `/deliverability/campaign?${defaultQueryParams.join('&')}&${exampleMetrics}`;
		const goodParams = {
			apiName: 'campaign',
			metricNames: new Set(['count_targeted', 'count_delivered', 'count_unique_confirmed_opened'])
		};
		...
		describe('When everything is great', () => {
			...
			describe('Fetch requests', () => {
				...
				it('passes on a custom `from` ISO datetime', () => {
					...
					const campaignUrlCustomFrom = `/deliverability/campaign?from=${customFrom}&${defaultTimezone}&${exampleMetrics}`;

					const nocked = nock(espMetricsBaseUrl)
						.get(campaignUrlCustomFrom)
						.reply(200, campaignFixtures.ok);
				...
				it('passes on requested metrics names to the API', () => {
					...
					const campaignUrlCustomMetrics = `/deliverability/campaign?${defaultFrom}&${defaultTimezone}&metrics=${customMetrics.join()}`;

					const nocked = nock(espMetricsBaseUrl)
						.get(campaignUrlCustomMetrics)
						.reply(200, campaignFixtures.ok);
				...
				it('passes on requested orderBy field to the API', () => {
					...
					const [defaultFrom, defaultTimezone] = defaultQueryParams;
					const campaignUrlCustomOrder = `/deliverability/campaign?${defaultFrom}&${defaultTimezone}&${exampleMetrics}&order_by=count_targeted`;

					const nocked = nock(espMetricsBaseUrl)
						.get(campaignUrlCustomOrder)
						.reply(200, campaignFixtures.ok);
		...
		describe('When not everything is great tbqh', () => {
			...
			it('rejects if the server responds with an error (how dare they)', () => {
				nock(espMetricsBaseUrl)
					.get(campaignUrl)
					.reply(500, campaignFixtures.serverError);
			...
			it('rejects if the server responds with a bad request error', () => {
				nock(espMetricsBaseUrl)
					.get(campaignUrl)
					.reply(400, campaignFixtures.badRequest);
			...
			it('rejects if the server responds as a teapot', () => {
				nock(espMetricsBaseUrl)
					.get(campaignUrl)
					.reply(418, campaignFixtures.serverError);
